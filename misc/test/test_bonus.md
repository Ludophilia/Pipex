...
```./pipex_bonus file1 "cmd1 args" "cmd2 args" file2```
...
- [x] `< /dev/random tr -cd '[:print:]' | fold -w 30 | head -15 | nl > /dev/stdout`
	- `./pipex_bonus /dev/random "tr -cd '[:print:]'" "fold -w 30" "head -15" nl /dev/stdout`
	- `valgrind ./pipex_bonus /dev/random "tr -cd '[:print:]'" "fold -w 30" "head -15" nl /dev/stdout`
...
- [x] `time (< /dev/stdin sleep 5 | sleep 5 | sleep 5 > /dev/stdout)`
	`time ./pipex_bonus /dev/stdin "sleep 5" "sleep 5" "sleep 5" /dev/stdout`
...
- [x] `< /dev/random yes YAAAAAAAS | yes YESYESYES | yes 42 > /dev/stdout`
	`./pipex_bonus /dev/random "yes YAAAAAAAS" "yes YESYESYES" "yes 42" /dev/stdout`
	`valgrind ./pipex_bonus /dev/random "yes YAAAAAAAS" "yes YESYESYES" "yes 42" /dev/stdout`
...
- [x] `< test/infile strings | nl | tee > /dev/stdout`
	`./pipex_bonus test/infile strings nl tee /dev/stdout`
	`valgrind ./pipex_bonus test/infile strings nl tee /dev/stdout`
...
- [x] `< /dev/urandom strings | nl | tee > /dev/stdout`
	`./pipex_bonus /dev/urandom strings nl tee /dev/stdout`
	`valgrind ./pipex_bonus /dev/urandom strings nl tee /dev/stdout`
...
- [x] `< /dev/random yes YAAAAAAAS | yes YESYESYES | head > /dev/stdout`
	`./pipex_bonus /dev/random "yes YAAAAAAAS" "yes YESYESYES" head /dev/stdout`
	`valgrind ./pipex_bonus /dev/random "yes YAAAAAAAS" "yes YESYESYES" head /dev/stdout`
	- [x] displays 10 `YESYESYES` on stdout
	- [x] `$SHELL` displays the prompt
	- [x] Leak protected?

- [x] `< /dev/random yes YAAAAAAAS | yes YESYESYES | strings | tee | head > /dev/stdout`
	`./pipex_bonus /dev/random "yes YAAAAAAAS" "yes YESYESYES" strings tee head /dev/stdout`
	`valgrind ./pipex_bonus /dev/random "yes YAAAAAAAS" "yes YESYESYES" strings tee head /dev/stdout`
	- [x] displays 10 `YESYESYES` on stdout
	- [x] `$SHELL` displays the prompt
	- [x] Leak protected?
...
- [ ] `<< EOF cat | strings | nl >> test/outfile`
	  `./pipex_bonus here_doc EOF cat strings nl test/outfile`
	  `valgrind ./pipex_bonus here_doc EOF cat strings nl test/outfile`
	- [x] User input: `123\n1234\n123\nEOF`
...
- [ ] `<< EOF cat | strings | nl >> test/outfile2`
	`./pipex_bonus here_doc EOF cat strings nl test/outfile2`
	`valgrind ./pipex_bonus here_doc EOF cat strings nl test/outfile2`
	- [x] User input: `123\n1234\n123\nEOF`
	- [x] Displays the input line by line if nb of chars >= 4 with the line 
	number before
	- [x] File created?
...
- [ ] `./pipex_bonus here_doc strings nl test/outfile`
	`valgrind ./pipex_bonus here_doc strings nl test/outfile`
...
- [ ] `./pipex_bonus here_doc test/outfile strings tee nl`
	`valgrind ./pipex_bonus here_doc test/outfile strings tee nl`
...
- [x] `./pipex_bonus here_doc strings tee test/outfile nl`
	`valgrind ./pipex_bonus here_doc strings tee test/outfile nl`
...
- [x] `< /dev/gf tee | cat | fold -w 1 > /dev/stdout`
	`./pipex_bonus /dev/gf tee cat "fold -w 1" /dev/stdout`
	- `valgrind ./pipex_bonus /dev/gf tee cat "fold -w 1" /dev/stdout`
...
- [x] `< test/infile tee | cat | tac > /dev/stdout`
	- `./pipex_bonus test/infile tee cat tac /dev/stdout`
	- `valgrind ./pipex_bonus test/infile tee cat tac /dev/stdout`
...
- [x] `< . tee | head -3 nl > /dev/stdout`
	- `./pipex_bonus . tee "head -3" nl /dev/stdout`
	- `valgrind ./pipex_bonus . tee "head -3" nl /dev/stdout`
	- [x] Expected error message
...
- [x] `< /dev/random test/fcmd | nl > /dev/stdout`
	- `./pipex_bonus /dev/random test/fcmd nl /dev/stdout`
	- `< /dev/random tee | test/fcmd > /dev/stdout`
	- `./pipex_bonus /dev/random tee test/fcmd /dev/stdout`
	- [x] `valgrind ./pipex_bonus /dev/random test/fcmd nl /dev/stdout`
	- [x] `valgrind ./pipex_bonus /dev/random tee test/fcmd nl /dev/stdout`
...
- [x] `< /dev/random abc | def | xyz > /dev/stdout`
	- [x] `valgrind ./pipex_bonus /dev/random abc def xyz /dev/stdout`
	- [x] Expected error message
...
- [x] `env -u PATH < /dev/random tee | head -c 80 | nl > /dev/stdout`
	- `env -u PATH ./pipex_bonus /dev/random tee "head -c 80" nl /dev/stdout`
	- `/usr/bin/valgrind ./pipex_bonus /dev/random tee "head -c 80" nl /dev/stdout`
	- [x] Expected error message
...
- [x] Expected error message
	- pipex: `<cmd>: command not found`
	- Examples:
		- `./pipex_bonus /dev/random "" "" /dev/stdout`
		- `valgrind ./pipex_bonus /dev/random "" "" /dev/stdout`
	- [x] Leak protected?
...
- [x] `< /dev/random strings | head | nl > test/outfile`
	- `./pipex_bonus /dev/random strings head nl test/outfile`
	- `valgrind ./pipex_bonus /dev/random strings head nl test/outfile`
...
- [x] `< /dev/random strings | head | nl > test/outfile2`
	- `./pipex_bonus /dev/random strings head nl test/outfile2`
	- `valgrind ./pipex_bonus /dev/random strings head nl test/outfile2`
...
- [x] `< /dev/random strings | head | nl | nl | fold -w 30 > test/foutfile`
	- `./pipex_bonus /dev/random strings head nl nl "fold -w 30" test/foutfile`
	- `valgrind ./pipex_bonus /dev/random strings head nl nl "fold -w 30" test/foutfile`
...
- [x] `./pipex_bonus /dev/random "tr -cd '[:print:]" "fold -w 30" "head -15" nl .`
- [x] `valgrind ./pipex_bonus /dev/random "tr -cd '[:print:]" "fold -w 30" "head -15" nl .`
	- [x] Expected error message
