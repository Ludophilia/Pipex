...
- [x] `< /dev/random cat | head -c 80 > /dev/stdout`
	...
	- [x] displays the first 80 bytes from random.
...
- [x] `< /dev/stdin cat | head -c 1 > /dev/stdout`
	...
	- [x] display 1 byte sent by the user, as `head` will close the pipe afterwards.
	- [x] `$SHELL` displays the prompt after 1 more try.
	- [x] Leak protected?
...
- [x] `< /dev/stdin cat | tee /dev/null > /dev/stdout`
	...
	- [x] wait for the user's input via `/dev/stdout` to start processing `cat`
	- [x] `cat` reads `/dev/stdin` and displays nothing on `stdout` as the result
	is sent to the write end of the pipe.
	- [x] `tee` reads `stdin` (the read end of the pipe) so the process will be 
	blocked until `cat` sends anything...
	- [x] `$SHELL` does not give back the prompt (send `EOF` via `ctrl+d`)
	- [x] Leak protected?
