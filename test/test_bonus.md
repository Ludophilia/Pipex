...
The bonus part is roughly similar to the mandatory part except in the fact
that it should simulate the shell script command below:

``` << here_doc cmd1 [args] | cmd2 [args] >> file2 ```

	- input to `cmd1` is managed via `here documents` instead of `file1`
	- the output of `cmd2` is appended to the already existing content
	if it exists of course, instead of just erasing it...
